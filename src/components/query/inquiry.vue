  ...
  methods: {
    ...
    async handleQuery() {
      ...
      try {
        ...
        console.log("Sending filtered request data to /admin/financial/loan-enquiry:", filteredData);
        
        // Make the API call
        const response = await axios.post(
          "https://crednow-app-t4vnc.ondigitalocean.app/api/v1/admin/financial/loan-enquiry?page=1&limit=10",
          filteredData,
          {
            headers: {
        ...
        console.log("Loan inquiry API Response:", response.data);
        
        // Emit the response data to parent component
        this.$emit("query", response.data);
      } catch (error) {
        console.error("Error during loan inquiry:", error);
        
        // Emit error to parent
        this.$emit("error", {
          success: false,
          message: error.response?.data?.message || error.message || "An error occurred while fetching loan data",
          error: error
        });
      } finally {
        this.loading = false;
    ...
    async fetchDataForPage(paginationParams) {
      ...
      try {
        ...
        const response = await axios.post(
          `https://crednow-app-t4vnc.ondigitalocean.app/api/v1/admin/financial/loan-enquiry?page=${paginationParams.page}&limit=${paginationParams.limit}`,
          params,
          {
            headers: {
              "Content-Type": "application/json",
        ...
        console.log("Paginated loan inquiry API Response:", response.data);
        
        // Emit the response data to parent component
        this.$emit("query", response.data);
      } catch (error) {
        console.error("Error during paginated loan inquiry:", error);
          ...
          if (key === 'daysOverDue_start' || key === 'daysOverDue_end') {
            ...
            if (!requestData.daysOverDue) {
              requestData.daysOverDue = {
                start: formData.daysOverDue_start ? parseInt(formData.daysOverDue_start) || null : null,
                end: formData.daysOverDue_end ? parseInt(formData.daysOverDue_end) || null : null
              };
              
              // If both start and end are null, don't include daysOverDue at all
              if (requestData.daysOverDue.start === null && requestData.daysOverDue.end === null) {
                delete requestData.daysOverDue;
              }
            }
          } else {
            ...
            if (typeof value === 'string' && !isNaN(value) && 
                ['amount', 'loanId', 'userId'].includes(key)) {
    ...
    handleReset() {
      ...
      // After reset, fetch all loans (empty query)
      this.handleQuery();
    },
